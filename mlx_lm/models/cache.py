...
class QuantizedKVCache(_BaseCache):
    ...
    def __init__(self, group_size: int = 64, k_bits: int = 8, v_bits: int = 4):
        ...
        self.k_bits = k_bits
        self.v_bits = v_bits
        # Kept for callers that probe `hasattr(cache, "bits")` to detect a
        # quantized cache; the K cache bit-width is the representative value.
        self.bits = k_bits
    ...
    def update_and_fetch(self, keys, values):
        ...
        if self.keys is None or (prev + num_steps) > self.keys[0].shape[-2]:
            el_per_int_k = 8 * mx.uint32.size // self.k_bits
            el_per_int_v = 8 * mx.uint32.size // self.v_bits
            new_steps = (self.step + num_steps - 1) // self.step * self.step
            shape = (B, n_kv_heads, new_steps)

            def init_quant(dim, el_per_int):
                return (
            ...
            if self.keys is not None:
                ...
                self.keys = init_quant(k_head_dim, el_per_int_k)
                self.values = init_quant(v_head_dim, el_per_int_v)

        self.offset += num_steps

        keys = mx.quantize(keys, group_size=self.group_size, bits=self.k_bits)
        values = mx.quantize(values, group_size=self.group_size, bits=self.v_bits)
    ...
    def meta_state(self):
        return tuple(
            map(str, (self.offset, self.group_size, self.k_bits, self.v_bits))
        )

    @meta_state.setter
    def meta_state(self, v):
        self.offset, self.group_size, self.k_bits, self.v_bits = map(int, v)
...
class KVCache(_BaseCache):
    ...
    def to_quantized(
        ...
        if v_bits is None:
            v_bits = bits
        quant_cache = QuantizedKVCache(group_size=group_size, k_bits=k_bits, v_bits=v_bits)
        quant_cache.offset = self.offset
        if self.keys is not None:
            quant_cache.keys = mx.quantize(self.keys, group_size=group_size, bits=k_bits)
            quant_cache.values = mx.quantize(
                self.values, group_size=group_size, bits=v_bits
            )
        return quant_cache
...
class BatchKVCache(_BaseCache):
    ...
    def to_quantized(
        ...
        if v_bits is None:
            v_bits = bits
        quant_cache = QuantizedKVCache(group_size=group_size, k_bits=k_bits, v_bits=v_bits)
        quant_cache.offset = int(self._idx)
        if self.keys is not None:
            keys = self.keys[..., : self._idx, :]
            values = self.values[..., : self._idx, :]
            quant_cache.keys = mx.quantize(keys, group_size=group_size, bits=k_bits)
            quant_cache.values = mx.quantize(values, group_size=group_size, bits=v_bits)
        return quant_cache
