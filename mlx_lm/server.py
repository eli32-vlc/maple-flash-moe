    ...
    def _load(self, model_path, adapter_path=None, draft_model_path=None):
        ...
        if self.is_distributed:
            ...
            model, tokenizer = load(
                ...
                trust_remote_code=self.cli_args.trust_remote_code,
                # Flash-MoE keeps the full checkpoint file-backed; load lazily so
                # the unselected experts never get paged into DRAM.
                lazy=self.cli_args.flash_moe,
            )
        ...
        # Flash-MoE: keep the full checkpoint on disk, materialize only a small
        # activated expert subset per MoE layer into DRAM (AFM 3 Core Advanced).
        if getattr(self.cli_args, "flash_moe", False):
            from mlx_lm.models.maple import prepare_flash_moe

            prepare_flash_moe(
                model,
                active=self.cli_args.active_experts,
    ...
    def _serve_single(self, request):
        ...
        try:
            ...
            for gen in stream_generate(
                ...
                kv_v_bits=self.cli_args.kv_v_bits
                if self.cli_args.kv_v_bits is not None
                else self.cli_args.kv_bits,
            ):
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def handle_completion(self, request: CompletionRequest, stop_words: List[str]):
        ...
        def keepalive_callback(processed, total):
            ...
            logging.info(
                f"Prompt processing: {processed}/{total} ({pct:.1f}%) — {tps:.1f} tok/s"
            )
            if self.stream:
        ...
        try:
            ...
            prefill_tps = prompt_tokens / prefill_time if prefill_time > 0 else 0.0
            logging.info(
                f"Prompt: {prompt_tokens} tokens, {prefill_tps:.3f} tokens-per-sec"
            )
        except Exception as e:
            self._set_completion_headers(404)
            self.end_headers()
        ...
        try:
            ...
            logging.info(
                f"Generation: {gen_tokens} tokens, {decode_tps:.3f} tokens-per-sec"
            )
        finally:
            ctx.stop()
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of bits for K (keys) KV-cache quantization. Defaults to no quantization.",
    )
    parser.add_argument(
        "--kv-v-bits",
        type=int,
        ...
        "when --kv-bits is set. Use --kv-bits 8 --kv-v-bits 4 for q8 keys / q4 values.",
    )
    parser.add_argument(
        "--flash-moe",
        action="store_true",
        help="Maple only: keep the full checkpoint file-backed and materialize only a "
        "small activated expert subset per MoE layer into DRAM (AFM 3 Core Advanced style). "
        "Reduces RAM at the cost of restricting routing to the activated experts.",
    )
    parser.add_argument(
        "--active-experts",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="flash-moe: always-resident shared experts within the activated set. Default: 2.",
    )
    parser.add_argument(
        "--reselect-every",
        type=int,
